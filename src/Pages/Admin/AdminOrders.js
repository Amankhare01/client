...
const AdminOrders = () => {
    ...
    const getorders = async () => {
      try {
        const { data } = await axios.get(
          "https://mernstackecommerce-production.up.railway.app/api/v1/auth/all-orders"
        );
        setorders(data);
      } catch (error) {
        console.log(error);
  ...
  const handleChange=async(orderId,value)=>{
    try {
      const {data} = await axios.put(`https://mernstackecommerce-production.up.railway.app/api/v1/auth/order-status/${orderId}`,{
        status:value,
      });
      getorders();
    } catch (error) {
                ...
                {o?.products?.map((p, i) => (
                  <div className="row m-2 p-4 card flex-row bg-secondary cartgd-2">
                    <div className="col-md-4 ">
                      <img
                        src={`https://mernstackecommerce-production.up.railway.app/v1/product/photo-category/${p._id}`}
